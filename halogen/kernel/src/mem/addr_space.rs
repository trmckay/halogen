...
impl<'a> AddressSpace<'a> {
    /// Create a new `AddressSpace`; fails if any components cannot be allocated
    ///
    /// Allocates a root page table and empty regions
    pub fn new(_id: usize) -> Option<AddressSpace<'a>> {
        todo!()
    }

    /// Populates memory regions with the loadable contents of an ELF executable
