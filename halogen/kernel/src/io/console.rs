...
/// Register a new `Write` struct as the debug console.
...
/// Kernel API for printing with a newline.
#[macro_export]
...
/// Firmware API for printing with. This is lockless, as opposed to `kprint!`.
#[macro_export]
macro_rules! fwprint {
    ($($arg:tt)*) => ($crate::io::console::fwprint(format_args!($($arg)*)).unwrap());
}

/// Firmware API for printing with a newline. This is lockless, as opposed to
/// `kprint!`.
#[macro_export]
