...
use crate::{is_mapping_aligned, mask_range, mem::palloc};
...
impl PageTable {
    ...
    pub fn new() -> &'static mut PageTable {
        let alloc = palloc(1).expect("could not allocate page");
    ...
    pub fn next_level(&self, n: usize) -> &'static mut PageTable {
    ...
    pub fn map(&mut self, virt_addr: usize, phys_addr: usize, level: MappingLevel, flags: usize) {
        ...
        if let MappingLevel::TwoMegabyte | MappingLevel::FourKilobyte = level {
            pt = pt.next_level(virt_addr.vpn(vpn_num));
            vpn_num -= 1;
        }
    ...
    pub fn translate(&self, _virt_addr: usize) -> Option<usize> {
        unimplemented!();
    }
}
